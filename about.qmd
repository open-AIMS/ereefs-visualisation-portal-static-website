...
equations of conservation of momentum, continuity and conservation of
...
and low frequency ocean currents. The external boundaries of the CSIRO
hydrodynamic model are forced by the [BOM ACCESS-R weather models](https://www.bom.gov.au/australia/charts/about/about_access.shtml)
and [BOM OMAP for boundary currents](https://www.bom.gov.au/oceanography/forecasts/). The GBR1 1 km
...
cause coral bleaching, the ocean currents that disperse larvae of corals
...
runoff into the marine space and the nutrient recycling processes
...
significant challenge is that the model data has over tens of millions
...
-   Presenting visualisations using pre-rendered videos. These videos
    ...
    be easily seen,
-   Generating aggregations over time (daily, monthly, seasonal, annual)
...
-   Regridding the model data from curvilinear data to a rectangular
    grid so that it can be used in desktop GIS applications,
...
-   Generating exposure maps to identify regions where organisms are
...
<br>[![CC BY SA](images/cc.png){.cc width="15" height="15"}](https://creativecommons.org/licenses/by-sa/3.0/au/) by [AIMS](//eatlas.org.au/node/2)](images/AIMS-eReefs-platform-overview.png)


:::{.figure-caption}
The GBR4 and GBR1 model data is generated by CSIRO. It is then pushed to
the National Computing Infrastructure (NCI) where it is made available
...
as maps and map videos. These are made available via the AIMS eReefs
...
can take a while and so users are emailed back when their extractions
are complete.
:::
...
All the datasets used to create the visualisations on this site are made
...
via the [AIMS eReefs THREDDS server](https://thredds.ereefs.aims.gov.au/) by either downloading the
